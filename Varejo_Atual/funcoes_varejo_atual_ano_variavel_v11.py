...
def gerar_pivot_table(tb_analitica, cenario):
    '''Constrói a pivot table do processo anterior (Excel VBA) que vai alimentar os modelos para o Varejo Atual
    
    Parameters:
    tb_analitica(Pandas DataFrame): Dataframe da TB_ANALITICA_PREVIA (arquivo em Excel)
    
    Returns:
    p_table(pandas DataFrame): Pivot Table em formato de Pandas Dataframe
    '''
    ...
    anos_list = []
    for j in tb_analitica['MES'].unique():
        j = str(j)
        ano = j[:4]
        anos_list.append(ano)
    num_anos = len(set(anos_list)) 
    
    p_table = pd.pivot_table(tb_analitica, 
                             index= ['CENARIO','HISTORICO_BP_MIS','SUBTIPO','SEGMENTO','LINHA'], 
                             columns = ['MES'], 
                             values=['VALOR'], 
                             aggfunc = 'sum',
                             fill_value = 0)
    ...
    p_table = p_table.drop(['CENARIO'], axis=1)
    p_table = p_table.groupby(['HISTORICO_BP_MIS', 'SUBTIPO', 'SEGMENTO', 'LINHA']).sum()
    p_table = p_table.reset_index()
    
    for i in mes:
        p_table = p_table.rename(columns={int(f'{y1}{i}') : f'{y1}{i}',
                                          int(f'{y0}{i}') : f'{y0}{i}',
                                          int(f'{y01}{i}') : f'{y01}{i}',
                                          int(f'{y02}{i}') : f'{y02}{i}',
                                          int(f'{y03}{i}') : f'{y03}{i}',
...
def gerar_tabela_rmodelo(rmodelo_raw, pivot_resultado, subtipos, segmentos, num_anos, cenario):
    ...
    # Get ano atual (y0), ano anterior (y1), ano seguinte (y01), ano seguinte posterior (y02), ano seguinte posterior posterior (y03)
    m0 = datetime.now().month
    if m0 == 1 and cenario.lower() in ('avance', 'avanc', 'avan', 'fechto', 'fechamento', 'fecham') or m0 == 1 and cenario in ('P27','Negócios','Prévia'):
        y0 =  datetime.now().year - 1
    else:
        y0 =  datetime.now().year 
    y1 = y0 - 1
    ...
    rmodelo_raw = rmodelo_raw.rename(columns={'Unnamed: 9': '',
                                              '%': f'Reparto % {y1}',
                                              '%.1': f'Reparto % {y0}'})

    rmodelo_raw = rmodelo_raw[['Totalizador', 'Chave Cascada', 'Alíquotas', 'Cascada', 'Conceito','Cod', 'Itens / Período', 
                               'Segmentos', 'Linha excel', '', f'Reparto % {y1}',f'Reparto % {y0}', 'ISS', 'PIS', 'IR']]
...
#     rmodelo_raw['Conceito'] = rmodelo_raw['Conceito'].replace('Fictício', 'TTI Total')    
    ...
    df1=df2=df3=df4=df5=df6=df7=df8=df9=df10=df11=df12=df13=df14=df15=df16=df17=df18=df19=df20=df21=df22=df23=df24=pd.DataFrame()
    df_list1 = [df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16,df17,df18,df19,df20,df21,df22,df23,df24]

    df_list2 = copy.deepcopy(df_list1)
    df_list3 = copy.deepcopy(df_list1)

    df_list_final = []
    

    # Preenchimento dos RModelos
    ...
    for i in range(len(list(segmentos.keys()))):
        dfficticio_filt = pivot_resultado.loc[(pivot_resultado['HISTORICO_BP_MIS'] == 'Resultado') & 
                     (pivot_resultado['SUBTIPO'] == subtipos[7]) & 
                     (pivot_resultado['SEGMENTO'] == list(segmentos.keys())[i])]        
        df_list3[i] = pd.merge(rmodelo_ficticio_raw, dfficticio_filt, left_on = ['Cod'], right_on = ['LINHA'], how = 'left')
        
    # Empilhar os 3 tipos de RModelo
...
#         df_list_final[i]['Conceito'] = df_list_final[i]['Conceito'].replace('Resultado contabil', 'Contábil')
#         df_list_final[i]['Conceito'] = df_list_final[i]['Conceito'].replace('TTI Total', 'Fictício')

    return df_list_final


def gerar_excel_rmodelo(rmodelo_final, segmentos):
    '''Gera o RModelo em Excel, em formato raw.
    ...
    writer = pd.ExcelWriter(f"Varejo_Atual_rmodelo_{now.year}-{now.month:02d}-{now.day:02d}.xlsx", engine="xlsxwriter")
    contador = 0
    for i in rmodelo_final:
        i.to_excel(writer, sheet_name = f'R_{list(segmentos.values())[contador]}', index = False)
        contador += 1
    writer.close()
...
def gerar_tabela_pmodelo(pmodelo_raw, pivot_resultado, subtipos, segmentos, num_anos, cenario):    
    ...
    df_list2 = copy.deepcopy(df_list1)
    df_list3 = copy.deepcopy(df_list1)
    df_list4 = copy.deepcopy(df_list1)
    df_list5 = copy.deepcopy(df_list1)
...
def gerar_excel_pmodelo(pmodelo_final, segmentos):
    '''Gera o PModelo em Excel, em formato raw.
    ...
    writer = pd.ExcelWriter(f"Varejo_Atual_pmodelo_{now.year}-{now.month:02d}-{now.day:02d}.xlsx", engine="xlsxwriter")
    contador = 0
    for i in pmodelo_final:
        i.to_excel(writer, sheet_name = f'P_{list(segmentos.values())[contador]}', index = False)
        contador += 1
    writer.close()
...
def gerar_excel_pmodelo_ponta(pmodelop_final, segmentos):
    '''Gera o PModelo Ponta em Excel, em formato raw.
    ...
    writer = pd.ExcelWriter(f"Varejo_Atual_pmodelop_{now.year}-{now.month:02d}-{now.day:02d}.xlsx", engine="xlsxwriter")
    contador = 0
    for i in pmodelop_final:
        i.to_excel(writer, sheet_name = f'P_{list(segmentos.values())[contador]}_Ponta', index = False)
        contador += 1
    writer.close()
...
def gerar_tabela_smodelo(pmodelo, rmodelo, smodelo_raw, dic_dias_mes, dic_du_mes, anos, num_anos, cenario):
    ...
    s_modelo_raw_p = copy.deepcopy(pmodelo)
    s_modelo_raw_rtot = copy.deepcopy(rmodelo)
    ...
    df1=df2=df3=df4=df5=df6=df7=df8=df9=df10=df11=df12=df13=df14=df15=df16=df17=df18=df19=df20=df21=df22=df23=df24=pd.DataFrame()
    s_modelo_raw_r = [df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16,df17,df18,df19,df20,df21,df22,df23,df24]

    smodelo_ant = smodelo = smodelo_final = copy.deepcopy(s_modelo_raw_r)
    ...
    return smodelo_final


def gerar_excel_smodelo(smodelo_final, segmentos):
    '''Gera o SModelo em Excel, em formato raw.
    ...
    writer = pd.ExcelWriter(f"Varejo_Atual_smodelo_{now.year}-{now.month:02d}-{now.day:02d}.xlsx", engine="xlsxwriter")
    contador = 0
    for i in smodelo_final:
        i.to_excel(writer, sheet_name = f'S_{list(segmentos.values())[contador]}', index = False)
        contador += 1
    writer.close()
...
def ajustar_row_cod_0(modelo):
    ...
    for i in range(len(modelo)):
        modelo[i].reset_index(drop=True, inplace=True)
    indices_zerados = modelo[0].index[modelo[0]['Cod'] == '0'].tolist()
    cols = modelo[0].columns
   
    for i in range(len(modelo)):
        for j in cols:
            for u in indices_zerados:
                modelo[i].loc[u, j] = ''
...
def gerar_tabela_cascmodelo(cmodelo_raw, rmodelo_final, num_anos, cenario):
    ...
    cod_chave_casc = [13,14,15,16,17,18,19,20,22,24,25,26,27,28,41,42,43,44,45,46,
    ...
    df1=df2=df3=df4=df5=df6=df7=df8=df9=df10=df11=df12=df13=df14=df15=df16=df17=df18=df19=df20=df21=df22=df23=df24=pd.DataFrame()
    rmodelo_chave_casc_agr = [df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16,df17,df18,df19,df20,df21,df22,df23,df24]

    rmodelo_casc = copy.deepcopy(rmodelo_chave_casc_agr)
    rmodelo_chave_casc = copy.deepcopy(rmodelo_chave_casc_agr)
    rmodelo_casc_agr = copy.deepcopy(rmodelo_chave_casc_agr)
    cmodelo_chave_casc = copy.deepcopy(rmodelo_chave_casc_agr)
    cmodelo_casc = copy.deepcopy(rmodelo_chave_casc_agr)
    rmodelo_tot = copy.deepcopy(rmodelo_chave_casc_agr)
    cmodelo_geral = copy.deepcopy(rmodelo_chave_casc_agr)
    
    
    rmodelo = copy.deepcopy(rmodelo_final)
    for i in range(len(rmodelo)):
        rmodelo[i] = rmodelo[i].iloc[:2925]
        
    rmodelo_casc = copy.deepcopy(rmodelo)
    rmodelo_chave_casc = copy.deepcopy(rmodelo)
    ...
    for i in range(len(cmodelo_geral)):
        ...
        #Gerar colunas para a linha Margem c/ Produtos
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 12] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 13)&
                                                                                         (cmodelo_geral[i]['Cod'] <= 28)].sum()

        #Gerar colunas para a linha Provisiones de Insolvencias
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 29] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 30)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 33)].sum()

        #Gerar colunas para a linha Alocação de Capital + Remun. Resultado
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 34] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 35)&
                                                                                             (cmodelo_geral[i]['Cod'] <=37)].sum()
        ...
        #Gerar colunas para a linha Margem de Intermediação (líquida)
            ...
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 40] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 41)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 65)].sum()
            ...
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 72] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 73)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 82)].sum()

            #Gerar colunas para a linha Outros Resultados Operacionais
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 83] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 84)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 90)].sum()
            ...
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 95] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 96)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 101)].sum()

            #Gerar colunas para a linha Gastos Gerais da Administração
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 102] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 103)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 106)].sum()
            ...
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 127] = bdi10 + cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 122)&
                                                                                             (cmodelo_geral[i]['Cod'] <= 126)].sum()
            ...
            cmodelo_geral[i][j].loc[cmodelo_geral[i]['Cod'] == 131] = cmodelo_geral[i][j].loc[(cmodelo_geral[i]['Cod'] >= 132)&
                                                                                              (cmodelo_geral[i]['Cod'] <= 134)].sum()  
    ...
    return cmodelo_geral

def gerar_excel_cmodelo(cmodelo_final, segmentos):
    '''Gera o CModelo em Excel, em formato raw.
    
    Parameters:
    cmodelo(Pandas DataFrame): CModelo final preenchido
    segmentos(dict): Segmentos oriundos do arquivo de variáveis (variaveis_var_atual)
    ...
    writer = pd.ExcelWriter(f"Varejo_Atual_smodelo_{now.year}-{now.month:02d}-{now.day:02d}.xlsx", engine="xlsxwriter")
    contador = 0
    for i in cmodelo_final:
        i.to_excel(writer, sheet_name = f'Casc_{list(segmentos.values())[contador]}', index = False)
        contador += 1
    writer.close()

def retirar_ponto_zero_col_cod_casc(row, col):
...
def ajustar_colunas_dfs(pmodelo, pmodelop, rmodelo, smodelo, cascmodelo, num_anos, cenario):
    ...
    # Get ano atual (y0), ano anterior (y1), ano seguinte (y01), ano seguinte posterior (y02), ano seguinte posterior posterior (y03)
    m0 =  datetime.now().month
    if m0 == 1 and cenario.lower() in ('avance', 'avanc', 'avan', 'fechto', 'fechamento', 'fecham', 'P27','Negócios') or m0 == 1 and cenario in ('P27','Negócios'):
        y0 =  datetime.now().year - 1
    else:
        y0 =  datetime.now().year
    y1 = y0 - 1
...
def gerar_headers_finais(tb_analitica, num_anos, cenario):
    ...
    header_s0 = copy.deepcopy(header_p0)
    ...
    header_r0 = copy.deepcopy(header_p0)
...
def ajustar_headers_finais(header_p, header_r, header_s, header_casc, pmodelo, pmodelop, rmodelo, smodelo, cmodelo, num_anos, cenario):
    ...
    df1=df2=df3=df4=df5=df6=df7=df8=df9=df10=df11=df12=df13=df14=df15=df16=df17=df18=df19=df20=df21=df22=df23=df24=pd.DataFrame()
    pmodelo_ajust = [df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16,df17,df18,df19,df20,df21,df22,df23,df24]
    
    pmodelop_ajust = copy.deepcopy(pmodelo_ajust)
    rmodelo_ajust = copy.deepcopy(pmodelo_ajust)
    smodelo_ajust = copy.deepcopy(pmodelo_ajust)
    cmodelo_ajust = copy.deepcopy(pmodelo_ajust)
    pmodelo_final0 = copy.deepcopy(pmodelo_ajust)
    pmodelop_final0 = copy.deepcopy(pmodelo_ajust)
    rmodelo_final0 = copy.deepcopy(pmodelo_ajust)
    smodelo_final0 = copy.deepcopy(pmodelo_ajust)
    cmodelo_final0 = copy.deepcopy(pmodelo_ajust) 
    pmodelo_final = copy.deepcopy(pmodelo_ajust)
    pmodelop_final = copy.deepcopy(pmodelo_ajust)
    rmodelo_final = copy.deepcopy(pmodelo_ajust)
    smodelo_final = copy.deepcopy(pmodelo_ajust)
    cmodelo_final = copy.deepcopy(pmodelo_ajust)   
...
def acrescentar_linhas_em_branco_renomear_colunas_data(pmodelo, pmodelop, rmodelo, smodelo, cascmodelo, num_anos, cenario):
    ...
    pmodelo_temp = copy.deepcopy(pmodelo)
    rmodelo_temp = copy.deepcopy(rmodelo)
    pmodelop_temp = copy.deepcopy(pmodelop)
    smodelo_temp = copy.deepcopy(smodelo)
    cascmodelo_temp = copy.deepcopy(cascmodelo)
    ...
    df1=df2=df3=df4=df5=df6=df7=df8=df9=df10=df11=df12=df13=df14=df15=df16=df17=df18=df19=df20=df21=df22=df23=df24=pd.DataFrame()
    pmodelo_final = [df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16,df17,df18,df19,df20,df21,df22,df23,df24]
    rmodelo_final = copy.deepcopy(pmodelo_final)
    pmodelop_final = copy.deepcopy(pmodelo_final)
    smodelo_final = copy.deepcopy(pmodelo_final)
    cascmodelo_final = copy.deepcopy(pmodelo_final)
    ...
    #Renomear colunas data. Ex:'202301' para 'Jan/23'
#     for i in range(len(pmodelo)):
#         for j in dict_mes.keys():
#             pmodelo_final[i] = pmodelo_final[i].rename(columns={str(f'{y0}{j}') : dict_mes.get(f'{y0}{j}'[-2:]) + '/' + f'{y0[2:4]}'})
#             pmodelo_final[i] = pmodelo_final[i].rename(columns={str(f'{y1}{j}') : dict_mes.get(f'{y1}{j}'[-2:]) + '/' + f'{y1[2:4]}'})
#             pmodelo_final[i] = pmodelo_final[i].rename(columns={str(f'{y01}{j}') : dict_mes.get(f'{y01}{j}'[-2:]) + '/' + f'{y01[2:4]}'})
#             pmodelo_final[i] = pmodelo_final[i].rename(columns={str(f'{y02}{j}') : dict_mes.get(f'{y02}{j}'[-2:]) + '/' + f'{y02[2:4]}'})
#             pmodelo_final[i] = pmodelo_final[i].rename(columns={str(f'{y03}{j}') : dict_mes.get(f'{y03}{j}'[-2:]) + '/' + f'{y03[2:4]}'})
            
#             pmodelop_final[i] = pmodelop_final[i].rename(columns={str(f'{y0}{j}') : dict_mes.get(f'{y0}{j}'[-2:]) + '/' + f'{y0[2:4]}'})
#             pmodelop_final[i] = pmodelop_final[i].rename(columns={str(f'{y1}{j}') : dict_mes.get(f'{y1}{j}'[-2:]) + '/' + f'{y1[2:4]}'})
#             pmodelop_final[i] = pmodelop_final[i].rename(columns={str(f'{y01}{j}') : dict_mes.get(f'{y01}{j}'[-2:]) + '/' + f'{y01[2:4]}'})
#             pmodelop_final[i] = pmodelop_final[i].rename(columns={str(f'{y02}{j}') : dict_mes.get(f'{y02}{j}'[-2:]) + '/' + f'{y02[2:4]}'})
#             pmodelop_final[i] = pmodelop_final[i].rename(columns={str(f'{y03}{j}') : dict_mes.get(f'{y03}{j}'[-2:]) + '/' + f'{y03[2:4]}'})
            
#             rmodelo_final[i] = rmodelo_final[i].rename(columns={str(f'{y0}{j}') : dict_mes.get(f'{y0}{j}'[-2:]) + '/' + f'{y0[2:4]}'})
#             rmodelo_final[i] = rmodelo_final[i].rename(columns={str(f'{y1}{j}') : dict_mes.get(f'{y1}{j}'[-2:]) + '/' + f'{y1[2:4]}'})
#             rmodelo_final[i] = rmodelo_final[i].rename(columns={str(f'{y01}{j}') : dict_mes.get(f'{y01}{j}'[-2:]) + '/' + f'{y01[2:4]}'})
#             rmodelo_final[i] = rmodelo_final[i].rename(columns={str(f'{y02}{j}') : dict_mes.get(f'{y02}{j}'[-2:]) + '/' + f'{y02[2:4]}'})
#             rmodelo_final[i] = rmodelo_final[i].rename(columns={str(f'{y03}{j}') : dict_mes.get(f'{y03}{j}'[-2:]) + '/' + f'{y03[2:4]}'})
            
#             smodelo_final[i] = smodelo_final[i].rename(columns={str(f'{y0}{j}') : dict_mes.get(f'{y0}{j}'[-2:]) + '/' + f'{y0[2:4]}'})
#             smodelo_final[i] = smodelo_final[i].rename(columns={str(f'{y1}{j}') : dict_mes.get(f'{y1}{j}'[-2:]) + '/' + f'{y1[2:4]}'})
#             smodelo_final[i] = smodelo_final[i].rename(columns={str(f'{y01}{j}') : dict_mes.get(f'{y01}{j}'[-2:]) + '/' + f'{y01[2:4]}'})
#             smodelo_final[i] = smodelo_final[i].rename(columns={str(f'{y02}{j}') : dict_mes.get(f'{y02}{j}'[-2:]) + '/' + f'{y02[2:4]}'})
#             smodelo_final[i] = smodelo_final[i].rename(columns={str(f'{y03}{j}') : dict_mes.get(f'{y03}{j}'[-2:]) + '/' + f'{y03[2:4]}'})
            
#             cascmodelo_final[i] = cascmodelo_final[i].rename(columns={str(f'{y0}{j}') : dict_mes.get(f'{y0}{j}'[-2:]) + '/' + f'{y0[2:4]}'})
#             cascmodelo_final[i] = cascmodelo_final[i].rename(columns={str(f'{y1}{j}') : dict_mes.get(f'{y1}{j}'[-2:]) + '/' + f'{y1[2:4]}'})
#             cascmodelo_final[i] = cascmodelo_final[i].rename(columns={str(f'{y01}{j}') : dict_mes.get(f'{y01}{j}'[-2:]) + '/' + f'{y01[2:4]}'})
#             cascmodelo_final[i] = cascmodelo_final[i].rename(columns={str(f'{y02}{j}') : dict_mes.get(f'{y02}{j}'[-2:]) + '/' + f'{y02[2:4]}'})
#             cascmodelo_final[i] = cascmodelo_final[i].rename(columns={str(f'{y03}{j}') : dict_mes.get(f'{y03}{j}'[-2:]) + '/' + f'{y03[2:4]}'})
...
def gerar_excel_var_atual(df_pnl_Var, df_pnl_VarTotal, pmodelo, pmodelop, rmodelo, smodelo, cascmodelo, segmentos, cenario, mes_ano_ref, versao):
    ...
    writer = pd.ExcelWriter(path_excel_raw, engine="xlsxwriter")
...
#     MoM_R01.to_excel(writer, sheet_name = 'anl_MoM_R01', index = False)
#     MoM_R02.to_excel(writer, sheet_name = 'anl_MoM_R02', index = False)
#     MoM_P01.to_excel(writer, sheet_name = 'anl_MoM_P01', index = False)
...
def personalizar_excel(path_excel_raw, cenario, mes_ano_ref, versao, num_anos):
    ...
    r_cod_cinza_medio_escuro = [78, 91, 100, 191, 193, 198, 204, 206, 207, 211, 215, 234, 302, 307, 311, 313, 334, 352, 373, 383, 395, 403, 432, 493, 503,
    ...
    backgroud_vermelho = PatternFill(patternType='solid', 
                                     fgColor='FF0000')
    
    backgroud_cinza_medio = PatternFill(patternType='solid', 
                                        fgColor='BFBFBF')
    
    backgroud_cinza_medio_escuro = PatternFill(patternType='solid', 
                                               fgColor='C0C0C0')
    
    backgroud_cinza_escuro = PatternFill(patternType='solid', 
                                         fgColor='595959')
    
    backgroud_cinza_claro = PatternFill(patternType='solid', 
                                        fgColor='D9D9D9')
    
    backgroud_verde_claro = PatternFill(patternType='solid', 
                                        fgColor='A9D08E')
    
    backgroud_amarelo = PatternFill(patternType='solid', 
                                    fgColor='FFFF00')
    
    fonte_branca_negrito = Font(name= 'Calibri',
                                size=11,
                                bold=True,
                                italic=False,
                                strike=False,
    ...
    fonte_branca = Font(name= 'Calibri',
                        size=8,
                        bold=False,
                        italic=False,
                        strike=False,
    ...
    fonte_preta_negrito = Font(name= 'Calibri',
                               size=11,
                               bold=True,
                               italic=False,
                               strike=False,
    ...
    wb = openpyxl.load_workbook(path_excel_raw)
    sheet_names = wb.sheetnames
    
    if num_anos == 2:
        p_head = ['A11','B11','C11','D11','E11','F11','G11','M11','N11','O11','P11','Q11','R11','S11','T11','U11','V11','W11','X11','Y11',
